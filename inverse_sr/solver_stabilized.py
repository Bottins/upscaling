...
@dataclass
class StabilizedInverseSolverConfig:
    scale: int = 4
    epochs: int = 250
    device: str = "cpu"
    ...
    freeze_forward_after_frac: float | None = 0.25
    freeze_prior_after_frac: float | None = 0.25


@dataclass
class StabilizedSolverResult:
    mode: str
    display_name: str
...
def _sigma_from_raw(raw: torch.Tensor, cfg: StabilizedInverseSolverConfig) -> torch.Tensor:
...
def _freeze_group_lrs(optimizer: torch.optim.Optimizer, frozen_groups: set[int]) -> None:
    for group_index in frozen_groups:
        optimizer.param_groups[group_index]["lr"] = 0.0


def solve_inverse_problem_stabilized(
    lr_observed: torch.Tensor,
    hr_reference: torch.Tensor,
    cfg: StabilizedInverseSolverConfig,
    mode: str,
    display_name: str | None = None,
    prior_weights: dict[str, float] | None = None,
    seed: int = 0,
) -> StabilizedSolverResult:
    ...
    forward_group_index = 1
    prior_group_index = 2 if prior_raw.numel() > 0 else None
    frozen_groups: set[int] = set()
    freeze_forward_epoch = None
    freeze_prior_epoch = None
    if cfg.freeze_forward_after_frac is not None:
        freeze_forward_epoch = max(1, int(cfg.epochs * cfg.freeze_forward_after_frac))
    if cfg.freeze_prior_after_frac is not None and prior_group_index is not None:
        freeze_prior_epoch = max(1, int(cfg.epochs * cfg.freeze_prior_after_frac))
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        charbonnier = torch.sqrt((predicted_lr - lr_observed).square() + cfg.charbonnier_eps ** 2).mean()
        ...
        if freeze_forward_epoch is not None and epoch >= freeze_forward_epoch:
            frozen_groups.add(forward_group_index)
        if freeze_prior_epoch is not None and epoch >= freeze_prior_epoch and prior_group_index is not None:
            frozen_groups.add(prior_group_index)
        if frozen_groups:
            _freeze_group_lrs(optimizer, frozen_groups)
        ...
        if epoch % cfg.log_every == 0 or epoch == 1 or epoch == cfg.epochs:
            ...
            print(
                ...
                f"freeze_fwd={'Y' if forward_group_index in frozen_groups else 'N'} "
                f"freeze_prior={'Y' if prior_group_index in frozen_groups else 'N'} "
                f"{prior_str}".rstrip()
            )
