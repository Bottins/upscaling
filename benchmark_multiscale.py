...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="x1/x2/x4 multiscale benchmark for inverse PINN with blind estimation of blur, noise, and prior weights."
    )
...
def _build_master_report(
    ...
    lines = [
        "# Inverse PINN Multiscale Benchmark",
        "",
        f"- Image: `{image_path}`",
...
def main() -> None:
    ...
    out_dir = Path(args.out) if args.out else Path("results") / f"multiscale_benchmark_{image_path.stem}_{stamp}"
    ensure_dir(out_dir)

    methods = default_configs()
    scenario_results: dict[str, dict] = {}
    ...
    print(f"\n[save] {out_dir / 'benchmark_report.md'}")
